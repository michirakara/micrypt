    ...
    match mode {
        CmdMode::Enc => {
            ...
            if key.is_none() {
                ...
                let key_length: i32 = rng.gen_range(3..=10);
            ...
            // encrypt
            let cipher_text: Vec<u8> = algorithms::vigenere::encrypt(&final_key, &vec_text);
            // output
        ...
        CmdMode::Dec => {
            ...
            let plain_text: Vec<u8> = algorithms::vigenere::decrypt(&final_key, &vec_text);
            // output
            println!("Key: {}", convert::vec_to_string(final_key));
